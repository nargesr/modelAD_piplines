...
In order to quantify the reads, you need to make indices first.
you only need to run indices once unless you are going to use different `fasta` or `GTF` files.

Data I used to make indices (mm39 and Gencode_vM32)
- [GTF link](https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_mouse/release_M32/gencode.vM32.chr_patch_hapl_scaff.annotation.gtf.gz)
- [Fasta link](https://hgdownload.soe.ucsc.edu/goldenPath/mm39/bigZips/mm39.fa.gz)
...
### Run Kallisto on bulk long-read RNA_seq data

This script will combine fastq files for each sample and then generate and submit separate jobs for each sample to run lr-Kallisto on bulk long read RNA_seq data to generate transcript-level count and tpm matrices: 
